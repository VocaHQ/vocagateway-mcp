...
| `delete_model` | `gateway:manage` | `DELETE /v1/admin/models/{id}` | Requires exact gateway and model confirmations; rejects active/downloading models |
...
the MCP process environment and is never returned to callers. Non-loopback
deployments require an HTTPS public endpoint. DNS-rebinding protection, Host and
Origin allowlists, and a one-MiB MCP request limit are enabled.
